...
// Runs once before the role-scoped projects. Signs in via Clerk's
// email-based ticket strategy (@clerk/testing looks the user up by email
// and mints a sign-in token through the Backend API — no password entry,
// no OTP, no UI form automation) and saves the resulting session so
// chromium-director can reuse it without signing in per-test. See
