...
5. Confirm `check` and `e2e-guest` pass on the PR, then **manually verify the feature on the PR's preview deployment URL** (the branch alias, which follows new commits). Each branch gets its own isolated Convex backend, auto-seeded with demo content and one Member per Role — so sign in as `e2e-admin+clerk_test@hugoforte.com` (or `-director`/`-chorister`) and you already hold that Role, with nothing to run by hand. Authenticated `chromium-director` E2E currently still runs only post-merge against production; moving it onto the PR's own preview backend is possible now and is tracked as a follow-up.
