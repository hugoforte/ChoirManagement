...
| Mint a **preview-type** deploy key (`preview:<team>:<project>\|…`) | **[API]** `POST /v1/projects/{project_id}/create_preview_deploy_key` | **Correction:** an earlier version of this file said dashboard-only. It isn't — it's in the [Management API](https://docs.convex.dev/management-api/create-preview-deploy-key). The **CLI** genuinely can't mint one (it only calls `/deployments/{name}/create_deploy_key`), which is what misled me. Auth is a Bearer **team access token**, and obtaining *that* is the one real manual step (dashboard → Team Settings → Access Tokens). |
| Create/seed/tear down a per-branch deployment | **[API]** team access token + `create_deployment` / `POST /v1/deployments/{name}/delete` | Caps: **40 deployments per team** on the free plan (300 Pro). Preview deployments auto-delete **5 days** after creation (14 on paid) and count toward that cap. Feature is beta. |
| Project-wide env var defaults (inherited by new deployments) | **[CLI]** `npx convex env default set K V` | The mechanism that lets a freshly provisioned preview deployment already know `CLERK_JWT_ISSUER_DOMAIN` etc. without a per-deployment setup step. |
| Seed a fresh preview deployment automatically | **[CLI]** `npx convex deploy --preview-run <fn>` | Fires **only** when the deploy is routed through a *preview* deploy key and provisions a new deployment — with a deployment-scoped key it's silently ignored (verified in the CLI bundle: `previewRun` is only invoked in the preview-key branch). Note a failing seed does **not** fail the deploy. |
