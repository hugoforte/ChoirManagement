...
| Build command | **[CLI]** commit it to `vercel.json` (`buildCommand`) | Preferred over the dashboard field: versioned, diffable, and readable by an agent with no dashboard access. |
...
| Deployment Protection / SSO | **[API]** project `ssoProtection` | Currently `all_except_custom_domains`, so **every** preview URL sits behind the Vercel SSO wall. Fine for the project owner reviewing their own previews; blocks everyone else. |
