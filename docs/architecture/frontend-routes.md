...
Per CONTEXT.md: **Admin** manages Members/Roles/choir settings; **Director** manages Music Library, Member roster, and Events; **Chorister** views the Music Library and RSVPs. `convex/lib/capabilities.ts` names five Capabilities from that prose — `manageEvents`, `manageLibrary`, `manageRoster`, `assignRoles`, `manageSettings` — with `requireCan` (backend) and `can` (frontend) both reading the same table, so a route's gate and its backing mutation's check can't drift apart the way two independent Role-list literals could.
